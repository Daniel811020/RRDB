...
def filter2D(imgs, kernels):
    ...
    imgs = tf.reshape(imgs, (h, w, c, 1))  # H x W x C x B
...
def random_mixed_kernels(kernel_size, kernel_props):
    kernel_type = random.choices(
        kernel_props["kernel_list"], kernel_props["kernel_prob"])[0]
...
def generate_poisson_noise(img, scale=1.0, gray_noise=0):
    ...
    if cal_gray_noise:
        ...
        img_gray = tf.clip_by_value(
            tf.math.round(img_gray * 255.0), 0, 255) / 255.

        vals_list = [
            len(tf.unique(tf.reshape(img_gray[i, :, :, :], h * w))[0]) for i in range(b)]

        vals_list = [
            2 ** tf.math.ceil(tf.math.log(tf.cast(vals, tf.float32)) / base_2) for vals in vals_list]
    ...
    vals_list = [len(tf.unique(tf.reshape(img[i, :, :, :], h * w * c))[0])
                 for i in range(b)]
    vals_list = [
        2 ** tf.math.ceil(tf.math.log(tf.cast(vals, tf.float32)) / base_2) for vals in vals_list]
...
def generate_gaussian_noise(img, sigma=10, gray_noise=0):
    ...
    if cal_gray_noise:
        noise_gray = tf.random.normal(
            [1, h, w, 1], dtype=img.dtype) * sigma / 255.
